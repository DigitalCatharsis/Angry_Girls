...
namespace Angry_Girls
{
    public class LaunchManager : MonoBehaviour
    {
        ...
        {
            if (!_canPressAtCharacters) { return; }

            if (clickedIndex < 0 || clickedIndex >= _charactersToLaunchLeft.Count)
                return;

            if (_charactersToLaunchLeft[clickedIndex] == CharacterToLaunch)
            {
                return;
            }

            SwapCharacters(clickedIndex, 0);
        ...
        private void Update()
        {
            ...
            {
                //Проверка на минимальную дистанцию
                if (_characterLauncher.IsLaunchDistanceSufficient())
                {
                    LaunchCharacter(CharacterToLaunch);
                    GameLoader.Instance.gameLogic_UIManager.launchPortraitUI.RemovePortraitAtIndex(0);
                }
                else
                {
                    // Слишком слабый запуск — отмена
                    _isLaunchAllowed = false;
                    _characterLauncher.CancelAiming();
                }
            }
